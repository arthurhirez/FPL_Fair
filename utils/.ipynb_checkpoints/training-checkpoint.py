...
def train(model: FederatedModel, private_dataset: FederatedDataset,
    ...
    if args.rand_dataset:
        ...
        selected_domain_dict = {'mnist': 6, 'usps': 4, 'svhn': 3, 'syn': 7, 'caltech': 3,'amazon': 2,'webcam':1, 'dslr': 4}  # base
        selected_domain_dict = {'mnist': 3, 'usps': 7, 'svhn': 6, 'syn': 4, 'caltech': 3,'amazon': 2,'webcam':1, 'dslr': 4}  # article
    ...
    for epoch_index in range(Epoch):
        ...
        if hasattr(model, 'loc_update'):
            epoch_loc_loss_dict = model.loc_update(pri_train_loaders, test_loaders)
